...
const agendaDay1 = [
  ...
  { time: "14:00", title: "Discovery que gera impacto", speaker: "Juliana Torres", tag: "Workshop" },
  { time: "16:00", title: "Painel: O futuro do PM no Brasil", speaker: "Vários speakers", tag: "Painel" },
];

const agendaDay2 = [
  { time: "09:00", title: "AI-First Product Strategy", speaker: "Rafael Costa", tag: "Keynote" },
  { time: "10:30", title: "Construindo times de alta performance", speaker: "Marina Silva", tag: "Palestra" },
  { time: "14:00", title: "De PM a CPO: carreira em produto", speaker: "Pedro Almeida", tag: "Fireside Chat" },
  { time: "16:30", title: "Encerramento & Networking", speaker: "", tag: "Networking" },
];

const AgendaItem = ({ item, index }: { item: typeof agendaDay1[0]; index: number }) => (
  <motion.div
    ...
    <section id="agenda" className="py-24 md:py-32 bg-card/50">
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-12">
          <div>
            <h3 className="font-display text-xl font-bold text-primary mb-6">Dia 1 — 18 de Setembro</h3>
            <div>
              {agendaDay1.map((item, i) => (
                <AgendaItem key={item.title} item={item} index={i} />
              ))}
            </div>
          </div>
          <div>
            <h3 className="font-display text-xl font-bold text-primary mb-6">Dia 2 — 19 de Setembro</h3>
            <div>
              {agendaDay2.map((item, i) => (
                <AgendaItem key={item.title} item={item} index={i} />
              ))}
            </div>
          </div>
